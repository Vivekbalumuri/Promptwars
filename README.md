...
*   **Perimeter Staff Balance:** Isolate an ingress row reporting a `CRITICAL` load state with a `RISING` trend marker. Click **Balance Load** or **Auto Re-route**. Show the algorithm instantly lowering the density score by $22\%$, reversing the vector indicator to `FALLING`, and automatically reallocating operator counts across rows.